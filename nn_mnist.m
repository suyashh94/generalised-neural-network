clear all;
%% Load training data
load('C:\Users\harla\Desktop\Matlab scripts\Datasets\training_data.mat');
...
%% Scaling the weights appropriately
% for i = 1:num_layers
%     if i == 1
%         W{i} = W{i} .* keep_prob_init;
%     else
%         W{i} = W{i} .* keep_prob;
%     end
% end
%% Load x_test
load('C:\Users\harla\Desktop\Matlab scripts\Datasets\test_data.mat');
x_test = im2double(x_test);

%% Predict y
y_predict_test = zeros(size(y_train,1),size(x_test,2));
[A_main,~] = forward_propagation(x_test,W,b,activation);
A_predict_test = A_main{end};
for i = 1:size(A_predict_test,2)
    [~,dummy] = max(A_predict_test(:,i));
    y_predict_test(dummy,i) = 1;
end
%%
...
subplot(2,1,2)
imagesc(y_predict_test)

%% error

error_test = (sum(abs(y_test - y_predict_test)));
error_test = length(find(error_test > 0))/ size(y_test,2);
...
y_test = y_train;
%% Predict y
y_predict_train = zeros(size(y_train,1),size(x_test,2));
[A_main,~] = forward_propagation(x_test,W,b,activation);
A_predict_train = A_main{end};
for i = 1:size(A_predict_train,2)
    [~,dummy] = max(A_predict_train(:,i));
    y_predict_train(dummy,i) = 1;
end
%%
...
subplot(2,1,2)
imagesc(y_predict_train)
%% error

error_train = (sum(abs(y_test - y_predict_train)));
error_train = length(find(error_train > 0))/ size(y_test,2);